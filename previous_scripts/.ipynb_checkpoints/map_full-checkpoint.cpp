    ...
    for(int side=0; side<2; side++)
    {
        ...
        for(int s=0; s<nSources; s++)
        {
            ...
            for(int i=0;i<nSources;i++)
            {
                ...
                if(i == s)
                {
                    color = kBlue+1;   // виділення поточного
                    size = 2.0;
                }
...
std::vector<std::pair<double,double>> source_positions =
    read_positions_from_file("source_positions.txt");

std::vector<std::pair<double,double>> om_positions =
    read_positions_from_file("om_positions.txt");
...
for(Long64_t ie=0; ie<maxEvents; ie++)
{
    ...
    for(int ip=0; ip<nParts; ip++)
    {
        ...
        // std::cout << ev.source_id << "  " << ev.om_id <<std::endl;
