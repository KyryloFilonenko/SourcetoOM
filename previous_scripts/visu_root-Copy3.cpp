    ...
    bool unifiedScale = true)
{
    ...
    // --- Конвертація фізичних координат -> display bin (wall/col) ---
    // Використовуємо позиції OM як референсну сітку (так само як source_maps).
    //
    // Italian side:  unique_y_neg[i] (зростаючий Y) відповідає wall i
    //                => display X для Italian = coord_to_idx(uY_neg, src_y)
    //
    // French side:   fill відбувається з інверсією (nWalls-1 - gid_wall),
    //                тому display X = coord_to_idx(uY_pos, -src_y)
    //                (дві інверсії взаємно скорочуються)
    //
    // Обидві сторони: display Y = coord_to_idx(uZ, src_z)

    auto coord_to_idx_gid = [](const std::vector<double>& vals, double val) -> int
    {
        int best = 0; double bestDist = std::abs(vals[0] - val);
        for(int i = 1; i < (int)vals.size(); i++)
        {
            double d = std::abs(vals[i] - val);
            if(d < bestDist){ bestDist = d; best = i; }
    ...
    // Унікальні відсортовані Y та Z з позицій OM
    ...
    dedup_v(uY_neg); dedup_v(uY_pos); dedup_v(uZ_gid);
    ...
    for(int side = 0; side < 2; side++)
    {
        ...
        for(int s = 0; s < nSources; s++)
        {
            ...
            // Підписи по X:
            //   Italian: бін ic=0 -> wall 0, ..., ic=19 -> wall 19
            //   French:  бін ic=0 -> wall 19 (фізично ліворуч),
            //             бін ic=19 -> wall 0  (фізично праворуч)
            //   В обох випадках підпис = реальний номер wall
            for(int ic = 0; ic < nWalls_gid; ic++)
            {
                int wall_label = isPos ? (nWalls_gid - 1 - ic) : ic;
                h->GetXaxis()->SetBinLabel(ic + 1, Form("%d", wall_label));
            }
            // Підписи по Y: col 0 внизу
            for(int ir = 0; ir < nRows_gid; ir++)
                h->GetYaxis()->SetBinLabel(ir + 1, Form("%d", ir));

            h->Draw("COLZ");

            // Сітка між комірками (суцільна, пунктирна)
            for(int ic = 0; ic <= nWalls_gid; ic++)
            {
                double x = colMin - 0.5 + ic;
                TLine* l = new TLine(x, rowMin - 0.5, x, rowMax + 0.5);
                l->SetLineColor(kBlack); l->SetLineWidth(2); l->SetLineStyle(2);
                l->Draw("same");
            }
            for(int ir = 0; ir <= nRows_gid; ir++)
            {
            ...
            // --- Маркери джерел (формат як у source_maps) ---
            // Поточне джерело s: червоний, більший; решта: зелений, менший
            {
                ...
                for(int i = 0; i < nSources; i++)
                {
                    double src_y_search = isPos ? -src_pos[i].first
                                                :  src_pos[i].first;
                    double mx = (double)coord_to_idx_gid(uY,     src_y_search);
                    double my = (double)coord_to_idx_gid(uZ_gid, src_pos[i].second);

                    int    mColor = (i == s) ? kRed      : kGreen + 1;
                    double mSize  = (i == s) ? 1.3       : 0.8;
                    double tSize  = (i == s) ? 0.045     : 0.0;

                    TGraph* gSrc = new TGraph(1, &mx, &my);
                    gSrc->SetMarkerStyle(20);
                    gSrc->SetMarkerSize(mSize);
                    gSrc->SetMarkerColor(mColor);
                    gSrc->Draw("P same");

                    TLatex* lsrc = new TLatex(mx + 0.15, my + 0.15, Form("%d", i));
                    lsrc->SetTextSize(tSize);
                    lsrc->SetTextColor(mColor);
                    lsrc->Draw("same");
                }
            }
            ...
            // Скидаємо межі для наступного використання
            h->SetMinimum(-1111);
            h->SetMaximum(-1111);
            delete c;
        }
